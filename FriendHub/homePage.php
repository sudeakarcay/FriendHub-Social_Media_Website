...
if (isset($_POST["postBtn"])) {
	...
	if (!($_FILES['file']['name'] == "")) {
		...
		if (!empty($textPost)) {
			...
			$msg = "Sorry, you can not upload an empty post.";
		}
	}
}
	...
	<div class="menu-bar">
		...
		<ul>
			<form id="searchForm" action="search.php" method="post">
				...
				<li><button class="button-2" type="submit" role="button"><img id='chatsvg' src="images/search.svg"></button></li>
			</form>
			...
			?>
				<ul>
					...
					if ($n['message'] != "Removed you from followers") { ?>
						<li><span data-name=<?php echo $n['user_name'] ?> data-surname=<?php echo $n['surname'] ?> data-fid=<?php echo $n['user_id'] ?> data-row=<?php echo $row; ?> class="fa-solid fa-circle-check"></span></li>
						<li><span data-fid=<?php echo $n['user_id'] ?> data-row=<?php echo $row; ?> class="fa-solid fa-circle-xmark"></span></li>
					<?php } else { ?>
						<li><span data-name=<?php echo $n['user_name'] ?> data-surname=<?php echo $n['surname'] ?> data-fid=<?php echo $n['user_id']  ?> data-row=<?php echo $row; ?> class="fa-solid fa-circle-check"></span></li>
		...
		<div class="post-area">
			<div class="share-area">
				<form action="" method="post" enctype="multipart/form-data">
					<textarea id="textarea" type="text" name="textPost" class="postText" placeholder="Share what you are thinking..."></textarea>
			...
			<div class='nextPost'>
					...
					<div class='post_bar'>
						...
						<div class='like-comment-part'>
							...
							<div class='comment'>
								...
								<img id='chatsvg' data-uid="<?= $p['id']; ?>" src='images/chat.svg'>
							</div>
				...
				if ($page > 1) {
					...
					echo "<form action='homePage.php' method='get'>
              		...
              		<button class='button-6' role='button'>< Previous Page</button>
         			 </form>";
				}
				if (count($posts) > ($start + $perPage)) {

					$nextPage = $page + 1;

					echo "<form action='homePage.php' method='get'>
              		<input type='hidden' name='page' value='$nextPage' />
              		<button class='button-6' role='button'>Next Page ></button>
         			 </form>";
				}
				echo "</div>";
